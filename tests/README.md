...
| `test_panel_page.py` | assertions against what the panel serves — the page (`emojikit.panel.PAGE`) and its scripts concatenated (`emojikit.panel.SCRIPT`, `emojikit.panel.SCRIPT_FILES`): drag-and-drop (every dragover edits the model and re-projects the grid, the drop records the dragstart snapshot instead of computing an index, a cancel restores it, the pointer's side of a tile decides before-or-after, a carried card is parked rather than unmounted), undo/redo, the virtual grid (only rows near the viewport exist, integer row arithmetic, an unchanged window costs a binary search), zoom (buttons, Ctrl+wheel, a typeable percentage, clamping, the top item anchored, compact below 75 %), the viewport observers (playback, and video sources attached only near the viewport), the scroll-time animation freeze, selection mode (a click drives the picks or `included` depending on whether it is on, the pick box owns its own click, and a picked card's ring is a rotating transform that stops under reduced motion), the holding area, the pack separators (bounded by the last actually-included item, not the raw array length) and the jump buttons. Served-text contracts supplement the real browser and SQLite scenarios in the browser suites |
