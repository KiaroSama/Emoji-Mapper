...
| `addStickerToSet failed: Bad Request: wrong file type` on an **animated** item | Telegram's `.tgs` validator refuses the file. It is stricter than "a gzipped Lottie": a **subtract mask** (`masksProperties[].mode == "s"`) is rejected, while add masks (`"a"`) are accepted. The file can be perfectly valid to every local check -- 512x512, correct fps, under the size cap, parses as JSON -- and still be refused. | Confirm it is the file and not the set with a single `uploadStickerFile` (`sticker_format=animated`), which touches no pack. Removing the subtract mask makes Telegram accept it, but it CHANGES THE ARTWORK (measured 23% of pixels on one real sticker), so it is the owner's call, not an automatic repair. |
