        ...
        def _route_post(self):
            ...
            if self.path == "/api/save":
                ...
                if not excluded <= scope:
                    self._send(400, b'{"error":"excluded keys must be inside known scope"}')
                    return
                ...
                with lock, Catalog(db_path) as cat:
                    ...
                    if not scope <= current.keys() or not scope <= visible:
                        self._send(409, b'{"error":"catalog identities or visibility changed; export the draft and reload"}')
                        return
