...
- **Emoji Mapper bot reply** reworked: sending one or more premium emoji (any
  ...
  ID per line for individual tap-to-copy, and (2) a collapsed quote of all IDs
  with a `copy_text` **“Copy all” inline button** for reliable one-click copy-all
  on every platform (a `<pre>` copy block can't collapse and a quote can't host a
  copy button, so the button bridges both). Both sections are collapsed
  (expandable) quotes; huge lists split across messages; a rejected custom emoji
  falls back to plain fallback chars.
